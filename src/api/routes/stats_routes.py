...
async def stats_summary():
    ...
    # farthest_direct is deliberately session-only (StatsReporter's live
    # in-memory accumulator), not backed by a DB fallback. Tried a
    # DB-backed fallback (matching by hop_start/hop_limit instead of
    # packet_type) and it surfaced a physically implausible 743 km
    # "direct" MeshCore contact -- hop_start=0/hop_limit=0 turns out not
    # to reliably mean "genuinely heard directly" across every packet
    # path (the live repeater-neighbour-poll rows default to the same
    # 0/0 and were only excluded by a separate, coincidental packet_id
    # convention). The live decode pipeline's real-time hop accounting
    # is the only trustworthy source for this stat -- if nothing direct
    # has been heard yet this session, showing nothing is more honest
    # than guessing from historical rows.
