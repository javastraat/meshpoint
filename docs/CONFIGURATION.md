...
dashboard:
  ...
  landing_page: "dashboard"  # page that loads with no hash in the URL -- "dashboard" or a "top"-category plugin's sidebar route (e.g. "reticulum-dashboard")
  ...
  web_terminal_toggle: false   # master switch for the Settings -> System card that sets the above
  map_tile_url: "https://tile.openstreetmap.org/{z}/{x}/{y}.png"  # Dashboard + Topology map tile source
```
...
**`web_terminal_toggle`** (default `false`; **no Settings-page control, no API route** — filesystem-only, same reasoning as `plugin_sources_enabled` above): the master switch for the Settings → System "Web terminal" card itself. `web_terminal_enabled`'s own on/off checkbox already sits behind admin auth + a confirm dialog + an audit log entry — but all of that only proves the *current* session consented, not that the session is trustworthy. A hijacked/phished admin session can tick the same confirm box, then trigger "Restart service" (also just an admin-audited action) to apply it — two API calls, no filesystem access, full root shell. `web_terminal_toggle` closes that for anyone who never opts in: until it's hand-set `true` in `local.yaml` and the service restarted, `PUT /api/config/dashboard` 403s on any attempt to change `web_terminal_enabled`, and the card doesn't render at all — not even a "this is disabled" note, so there's no hint the feature exists. This is a deliberate two-flag design rather than making `web_terminal_enabled` itself filesystem-only: once you've opted in here, the existing checkbox keeps working normally so you're not doing an SSH round-trip every time you want the terminal on or off for a bit. The tradeoff is accepted knowingly — after opting in, a compromised session can still flip `web_terminal_enabled` + restart exactly as before. What this closes is the (likely larger) population who never touch the terminal feature at all.

**`map_tile_url`** (default: the public OSM tile server, matching this app's behavior before this setting existed; also settable via `PUT /api/config/dashboard`, applies immediately — no restart, unlike `web_terminal_enabled`): the Leaflet tile URL template (`{z}`/`{x}`/`{y}` placeholders) for both the Dashboard and Topology maps, read fresh by `frontend/js/map_tile_source.js` every time either map is created rather than hardcoded per-file. Override it to read tiles from disk instead of the internet — the community `offline-map` plugin (`meshpoint-plugins/apps/offline-map`) downloads OSM tiles for an area and serves already-downloaded ones at `/api/offline-map/tiles/<collection>/<style>/{z}/{x}/{y}.png` without needing its own downloader process running, so pointing this at that URL keeps both maps working with no internet reachable at all — the actual point of the feature (emergency/off-grid use). The plugin's own Settings → Plugins page has a "Dashboard map source" picker that sets this for you from whatever's already been downloaded, rather than hand-typing the URL. A blank value, or one missing any of `{z}`/`{x}`/`{y}`, is rejected by the update endpoint.

**`landing_page`** (default `"dashboard"`; also a picker in Settings → System → "Landing page", applies immediately — no restart): which page opens with no hash in the URL — a fresh visit, or a bookmark to the bare dashboard URL. Choices are the built-in Dashboard plus any currently-enabled plugin whose `[sidebar]` table uses `category = "top"` (the built-in Dashboard's own sidebar tier, above "Networks" — see `KNOWN_SIDEBAR_CATEGORIES` in `src/plugins/manifest.py`) — e.g. the bundled `reticulum-dashboard` plugin. Deliberately *not* a picker over every sidebar page: most built-in tabs (Meshtastic, MeshCore, LoRaWAN, …) hide themselves entirely with no matching radio/companion configured, so landing there could show a blank or hidden page — the "top" tier is specifically the set of pages meant to work as a self-contained home screen. `PUT /api/config/dashboard` rejects anything else, and if a plugin set here is later disabled, the server ignores the stale value at serve time and falls back to `"dashboard"` rather than routing to a page that was never mounted.
...
Since there's no real CA, every browser shows the expected "this certificate is self-signed" warning the first time it visits each address — that's inherent to a LAN device with no public DNS name, not a bug. Restart to apply after flipping `tls_enabled`.

**`tls_port`** (default `8443`): a single socket can't serve both plain HTTP and HTTPS, so the real dashboard moves to this port once `tls_enabled` is on — `port` (`8080`) becomes a minimal redirect-only listener that 308s every request to the same host and path on `tls_port`. This means old `http://<address>:8080` bookmarks and QR codes still work — they just bounce straight to `https://<address>:8443` — no need to update them by hand. The redirect follows whichever address the request actually came in on (LAN IP, VPN IP, `.local` hostname — matching the cert's own SAN list), not a fixed one. If `tls_port` itself can't be bound (already in use, bad value), TLS is disabled for that run and the dashboard falls back to plain HTTP on `port` — same crash-avoidance reasoning as everywhere else in this section.
