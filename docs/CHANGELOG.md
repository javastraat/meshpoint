...
- **Real region/frequency/SF/bandwidth/coding-rate for `serial` Meshtastic capture, plus a topbar badge per USB stick.** The `serial` source previously stamped every packet with a hardcoded `906.875 MHz` (a US915 placeholder) and SF11/BW250 regardless of the connected stick's actual region or preset. It now reads the stick's real config at connect time (meshtastic-python completes this handshake automatically before the connection call returns) — region, channel, modem preset (or the raw custom SF/BW/coding-rate when not using a named preset) — the same "read once at connect, mirrors MeshCore's `self_info` fix" pattern. Frequency is only ever a specific number when the node's `channel_num` is 0 (the default/hash-derived channel) and the region is known (e.g. 433.875 MHz for EU_433 LongFast); a custom channel reports `0.0` (this codebase's existing "unknown" sentinel) rather than guessing. New topbar badge — one small cyan chip per configured Meshtastic USB stick (connected lamp, region, frequency) — separate from the single "primary" MeshCore companion chip, since every serial device is capture-only (no TX binding to pick "the" one). New `scripts/backfill_serial_meshtastic_signal.py` corrects old rows still carrying the `906.875` placeholder.
