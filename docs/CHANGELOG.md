...
- **Recent updates history.** The Updates page lists the last five apply/rollback events on this box — when, branch, from → to SHA, and success or the failed step — persisted in `data/update_history.json` (last 20 kept) and served by `GET /api/update/history`. Git log shows what commits exist; this shows when this gateway actually took them, including failed attempts.
