...
- **Topbar chips agree on "Reconnecting…".** When the dashboard connection drops, the Meshtastic USB chip now says "Reconnecting…" like the MeshCore chip, instead of blanking to dashes — in the same plain style, not the chip's spaced-out call-sign lettering. The chip also dropped its region segment (EU_433 etc.): the frequency already tells the band.
- **Topbar chips unified.** All three protocol chips now render their name slot in the same plain style (the concentrator chip's node name and the USB chip's node ID were spaced-out glowing call-sign lettering; MeshCore's was plain), and the concentrator chip dropped its EU_868 region segment too — the frequency already tells the band.
