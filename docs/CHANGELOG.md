...
- **`meshpoint report` works again.** The CLI report had been broken since API auth landed — every request got 401 and the command misreported "service is not running". It now prompts for dashboard admin credentials and uses a session cookie; `meshpoint status` distinguishes "running, login required" from "unreachable".
