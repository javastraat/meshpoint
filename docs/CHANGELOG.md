...
- **Check for updates survives the fan-control chown.** The sudo auto-detection above stat'ed the repo's top directory, which the service unit now chowns to the service user (lgpio needs to create its fan-control pipe there) — the dashboard then ran plain git against the still-root-owned `.git` and Check for updates failed with "cannot open '.git/FETCH_HEAD': Permission denied". Ownership is now probed on `.git` itself, the directory fetch actually writes to, so the gateway correctly goes back to sudo git.
#### Backup and restore
