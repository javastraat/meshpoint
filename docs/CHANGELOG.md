...
- **Band tag on node cards and the drawer.** Nodes now show a "433 MHz" / "868 MHz" chip derived from their latest packet's labelled capture source (`serial_433`, `meshcore_usb_868`, `concentrator`, …), so a 433/868 multi-stick setup is easy to tell apart at a glance. Deliberately not derived from `frequency_mhz`: the `serial` Meshtastic USB source stamps a hardcoded placeholder frequency regardless of the stick's real firmware region, so only the config-driven capture-source label is trustworthy here.
