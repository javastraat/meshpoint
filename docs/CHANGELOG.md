...
- **Backup and restore.** Settings → System downloads a timestamped `.tar.gz` of `config/local.yaml` and the full `data/` directory (SQLite hot snapshot, PKI keys, rollback state). Restore uploads a prior backup, stashes the current state on the Pi, and restarts the service. Restore drops stale SQLite `-wal` / `-shm` / `-journal` sidecars so a post-wipe restore cannot resurrect an empty node table from disk. Archive is not encrypted: store offline and keep it private.
