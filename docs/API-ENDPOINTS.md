...
Only one of Radio / Pagers / POCSAG / DAB+ may hold the RTL-SDR dongle at a time (the P2000, RTL433, ADS-B and ACARS plugins join that set when installed -- see their own `plugins/apps/*/README.md`).
...
| GET | `/api/pagers/status` · `/api/pocsag/status` | Viewer | Decoder state: running, frequency, decoded messages |
| POST | `/api/pagers/start` · `/api/pocsag/start` | Admin | Start the given decoder |
| POST | `/api/pagers/stop` · `/api/pocsag/stop` | Admin | Stop the given decoder |
