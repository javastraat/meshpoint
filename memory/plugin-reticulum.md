...
- **NomadNet browsing — IN the reticulum plugin** (decided: needs the same RNS attach LxmfService gives, so not a separate plugin). `memory/nomad-plugin.md` has the traced mechanics.
  ...
  - [ ] **N4 — polish** (later) — file downloads (`/file/…` path, `io.BufferedReader` response), timeout tuning for LoRa-only nodes (meshchat's 15s may be short over multi-hop LoRa), maybe a Micron light/dark toggle. Docs (PLUGINS/API-ENDPOINTS/README/CONFIGURATION Browse mention).
