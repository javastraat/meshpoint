...
- [ ] **Next: NomadNet browsing plugin** — `memory/nomad-plugin.md` is the plan. `"routes"` + `"hook"` (host = "reticulum"), needs `reticulum_panel.js` to call `window.mountPageHooks('reticulum', …)`, own RNS attach (option 1), Micron→HTML parser port is the big piece. Its own phased effort.
