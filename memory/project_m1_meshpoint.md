...
| W17 | Open | S-M | GitHub update-check badge on the dashboard — passive indicator (e.g. sidebar/topbar Settings nav item) showing an update is available, rather than only surfacing this after the user manually clicks "Check for updates". Existing self-update system (`update_check.py`/`update_routes.py`, "Check for updates"/"Latest commits" UI) already knows how to compare local vs remote — this just needs a periodic background check + a badge that lights up when `update_available` is true. User request 2026-07-12, added to wishlist, not yet built |
