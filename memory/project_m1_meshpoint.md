...
| — | Live test pending | XS | **Server-side Trends-chart downsampling** (see DONE note above) — built and verified against a DB copy, not yet live on the Pi |
| — | Live test pending | XS | **Telemetry auto-pruning + new `max_telemetry_retained` field** (see DONE note above) — built and stub-verified, not yet live on the Pi |
| — | Not investigated | XS | **8 stray `concentrator_backup_*.db` files on the Pi (~14MB), origin unknown** — grepped the whole repo, no code creates this naming pattern; ask the user if they made these manually, and if so whether they want a small rotation/prune habit (keep last N) instead of them accumulating forever |
