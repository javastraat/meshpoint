...
| — | DONE 2026-07-12 | XS | **Installer builds rtl_433** (generic RTL-SDR OOK/FSK decoder, merbanan/rtl_433 upstream). User gave the exact build commands (clone → `mkdir build` → `cmake ../` → `make`/`make install`, matching the redsea/multimon-ng from-source pattern already established); added as new section 9 in `scripts/install.sh` (right after multimon-ng, before the SX1302 HAL build), linking against the from-source `librtlsdr` built in section 6. Idempotent (`command -v rtl_433` check), same convention as the other from-source builds. Every subsequent section renumbered 10→21 (was 9→20) — `bash -n` syntax-checked after the renumber. CHANGELOG bullet + a new README "rtl_433 (optional, installer-only for now)" bullet added, explicitly noting no dashboard tab/API exists yet — this is currently just a CLI tool on the Pi, a possible future foundation for a broader-coverage RF-decoder tab (weather stations, TPMS, etc.) beyond the paging-only P2000/Pagers/POCSAG tabs. Not yet live-verified (no dashboard integration to test yet, and the build itself hasn't been run on the Pi). |
| — | Open | S | Retest `scripts/install.sh` on a fresh microSD flash — exercises everything added this session in one shot (RTL-SDR, redsea, multimon-ng, rtl_433, fastfetch banner, section renumbering); first fresh-flash run already passed once, user wants to test again |
