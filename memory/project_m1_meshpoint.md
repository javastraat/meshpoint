...
Verified: `bash -n` on install.sh; Python `ast.parse` on the two new backend files plus `server.py`; a stubbed-logic script exercising `AdsbListener`'s `/data.json`→status transform (flight-name trimming, `validposition`/`validtrack` nulling, seen-ascending sort) against a fake dump1090 payload — output shape confirmed correct; `node --check` on all three touched/new JS files; `ChangelogParser.parse_file()` confirmed the new CHANGELOG bullet still parses. Not yet Pi-verified live (no hardware antenna test this session) — next step if wanted is running `dump1090 --net --net-http-port 8081` manually on the Pi with a real antenna to confirm `/data.json` actually returns aircraft, then exercising the ADS-B tab's Start button end to end.
