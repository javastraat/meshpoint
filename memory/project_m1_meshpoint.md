...
| Open | M | **GUI-triggered DAB channel scan → persisted JSON presets** — let a scan run from the dashboard (background task, ~13-19 min) and write results to disk instead of hand-curating the preset array. Partial progress 2026-07-19: `scripts/dab_channel_scan.py` (the standalone CLI version) now writes a `--output`/`-o` JSON file (default `dab_channel_scan.json`) with every scanned channel's ensemble/SNR/stations, verified via a stubbed dry run (fake `scan_channel` results, confirmed JSON shape). Further same-day follow-ups, all verified against real scan output the user pasted: `ENSEMBLE_LABEL_OVERRIDES` rewrites the generic literal label `"DAB+"` (seen on 11C) to `"Commercial"`; `strip_channel_code()` removes a channel's own code when it's baked into the label (e.g. 8B's `"8B N-H / Flevo"` → `"N-H / Flevo"`, works for whichever channel is scanned, not hardcoded to 8B); `NO_LABEL_PLACEHOLDER` flags channels whose ensemble broadcasts no real name at all (9C decodes as literally just `"9C"`) as `"9C (no name broadcast -- set label in config)"` instead of silently showing the bare channel code as if it were a real name. Also same-day: `--output`'s default moved from a bare `dab_channel_scan.json` (whatever cwd it's run from) to a fixed `/opt/meshpoint/config/dab_channel_scan.json` -- user's request, so the future dashboard-consumption half of this item has a predictable place to read from -- with `os.makedirs(..., exist_ok=True)` added so a missing `config/` dir on a fresh install doesn't break the write. Also same-day, user's idea after noticing rescans of single channels were wiping the rest of the file: results now MERGE into `--output` by default (upsert by channel code, sorted back into raster order via new `channel_sort_key()`, untouched channels left exactly as they were) instead of overwriting, so a full station list can be built up across many small/targeted runs; new `--new` flag discards the file for a genuine fresh start; each channel entry gained its own `scanned_at` (top-level field renamed `last_run_at` since it now only describes the most recent run, not every merged entry). Verified end to end with a real multi-run simulation (2-channel scan → merge in a 3rd → in-place update of an already-present channel → `--new` wipes and restarts), not just unit-level checks. **Caught and fixed a real correctness gap from the user's own live data same day**: the per-channel upsert above was still a flat overwrite, so a rescan finding fewer stations than before (real observed case: `9C` dropped from 15 stations to 8 on a rescan, since the scan exits early once it finds *any* station) or one that failed entirely would silently erase better existing data -- the opposite of the whole point of merging. New `merge_channel_result()` makes it additive: station lists union (dedup, order-preserving) instead of replace, and a "nothing" rescan is a full no-op instead of overwriting known-good data with a blank result. Verified against the exact reported 9C scenario (rescan-to-8 no longer shrinks the merged 15) plus edge cases (new station appends, nothing-rescan is a no-op, previously-empty-placeholder channel correctly picks up real content). User explicitly scoped all of this to "just change the python script for now" — the GUI/background-task/dashboard-consumption half of this item is still not built. **2026-07-19, dashboard-consumption phase 1 built** (user: "so we get a 7th tab, named dab+ config... show the found channels and there names and make it possible to edit them, this is to start"): new **DAB+ Config tab** (7th Listener sub-tab, read-only against the RTL-SDR dongle -- no `sdr_registry` claim). Backend: `GET /api/dab/scan-results` (viewer-open) reads `config/dab_channel_scan.json` via `resolve_meshpoint_root()` (Mac-dev-vs-Pi-portable, same technique as `repo_source.py`), 404s with a clear message telling the operator to run the scan script first if missing (explicit user ask: "when there is no json found tell the user"); `PUT /api/dab/scan-results/{channel}/name` (admin-gated) sets/clears a `custom_name` override without touching the scanned `ensemble`/`snr`/`stations`. **Caught before shipping**: `merge_channel_result()` in the scan script rebuilt each merged entry from only its own 5 known fields, which would have silently dropped a web-UI-set `custom_name` the next time that channel got rescanned -- fixed to start from a copy of the existing entry so unknown extra fields survive. New `frontend/js/dab_config_panel.js`, wired into `listener_panel.js`'s tab-dispatch `if/else` chain (no central tab registry -- a missed branch would have silently fallen through to the Radio tab's status-polling default). **Also found and fixed in passing**: the user's own Mac copy of `config/dab_channel_scan.json` was missing its final `}` (lost in copy-paste from a terminal prompt that directly abutted the closing brace) -- repaired, confirmed valid JSON. Verification depth: backend GET/PUT logic stub-tested (no fastapi on Mac, per CLAUDE.md) including a full round-trip against a scratch copy of the REAL repaired file; the actual `_showActiveTab`/`_switchTab` dispatch logic extracted from the real file and exercised in a live V8 context (confirmed switching to/from the new tab doesn't fall through to Radio's polling branch); the real `_rowHtml`/`_esc` rendering logic likewise extracted and exercised against representative real data (custom_name fallback chain, custom badge, editing-state input, station-list escaping). `node --check` clean on all touched JS, CSS brace-balance verified, CHANGELOG re-parses (26 sections). **Explicit two-phase plan from the user, phase 2 NOT built yet**: make the actual DAB+ tab's hardcoded `DAB_CHANNEL_PRESETS` array in `dab_panel.js` read from this same JSON instead, so channel numbers/names/edits apply everywhere regardless of antenna location -- "after this we will dig deeper" |
