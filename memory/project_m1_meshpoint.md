...
| T18 | IN PROGRESS 2026-07-09 | S | **SenseCap M1 onboard LED/button/fan GPIO probe.** User wants to use the expansion-board LED for incoming-message notification, the CPU button for double-tap-reboot/triple-tap-shutdown, and the fan for temperature-driven control — asked first for the GPIO numbers and a test script, since no schematic is publicly available for this board. Researched via WebSearch/WebFetch (no existing repo docs cover this — `HARDWARE-MATRIX.md`/`TROUBLESHOOTING.md` only document the SX1302 concentrator's reset GPIO, 17/25, confirmed no collision with the pins below): **GPIO 22 (LED)** is corroborated by two independent sources (a community teardown blog + a Seeed `dtoverlay=gpio-led,gpio=22,label=lorawan,trigger=heartbeat` config snippet) — reasonably trustworthy. **GPIO 13 (button)** and **GPIO 14 (fan)** are UNVERIFIED — the same teardown blog explicitly says "the pinout of user button is currently unknown" and gives no GPIO for the fan; the user supplied 13/14 themselves from a source I couldn't independently confirm (official Seeed wiki Overview/FAQ pages have no GPIO specs at all, just block diagrams). New `scripts/test_gpio_hardware.py` (uses `gpiozero`, preinstalled on Raspberry Pi OS, NOT added as a project dependency since it's a standalone diagnostic, not part of the app) — deliberately cautious given 2 of 3 pins are unverified: LED test just blinks (low risk, well-corroborated pin); button test is read-only (safe regardless of correctness); fan test requires an explicit Enter keypress before driving the pin, with a printed warning to watch/listen and Ctrl+C if anything unexpected happens elsewhere on the board. All three pins are overridable via `--led-pin`/`--button-pin`/`--fan-pin` flags so wrong guesses can be retried without editing the script. NOT yet run on the Pi — next step is the user running `python3 scripts/test_gpio_hardware.py all` (or each target individually) to confirm which pins are actually correct before any real feature (message-triggered LED, multi-tap button actions, temp-driven fan control) gets built — those are good follow-on ideas but explicitly NOT built yet, since they'd be built on unverified pin guesses for 2 of the 3 peripherals. |
