...
| — | Open | S | Run `scripts/dab_channel_scan.py` on the Pi (stop any active RTL-SDR tab first) to find which local/regional DAB channels actually reach this Amsterdam antenna — replaces the guesswork that led to the dead `6C` preset. Then decide whether to wire a "scan all channels" button into the DAB+ tab itself (user: "first on the pi maybe then in the gui") |
