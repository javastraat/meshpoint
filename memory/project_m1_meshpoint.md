...
| W5 | BUILT 2026-07-13, not yet Pi-verified | M-L | **DAB+ tab via welle-cli.** See closure paragraph below. |
| — | Open | S | Retest DAB+ end-to-end on the real Pi (the `DabListener`/`/api/dab/*`/`DabPanel` build is only stub-verified on the Mac so far — raw `welle-cli` itself was manually confirmed working live, but not through Meshpoint's own code) |
| — | Open | S | Extend `SidebarTelemetryRail` (mini radio player) to also cover DAB+ — deferred on purpose to prove the DAB+ tab itself first, same build order as the original FM sidebar player |
| — | Open | XS | Add `welle.io` install step to `scripts/install.sh` once DAB+ is Pi-verified (currently manual-only, documented in README) |
