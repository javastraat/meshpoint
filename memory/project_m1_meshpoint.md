...
User reaction after the dropdown rewrite: "nice :)" — no follow-up issues raised. NOT yet Pi-verified live — next step is the user pulling this, restarting, and confirming: (1) the Updates page's Upstream card now shows/links `javastraat/meshpoint @ main` instead of the old generic label, (2) selecting "Custom branch" populates the dropdown with real branch names from a live GitHub API call (not just the cached/mocked test data) and auto-selects `main` since that's the live install branch, (3) as a sanity check that the fallback actually works as intended, could temporarily test on a checkout with no `origin` remote or a non-GitHub one and confirm it lands on `KMX415/meshpoint` rather than erroring.
