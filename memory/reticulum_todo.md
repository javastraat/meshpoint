...
| ~~7~~ | ~~**Add Reticulum (and Pager) to the Messages page's protocol filter chips**~~ | build | Low | Low–Med | Low | **BUILT 2026-09-16** — added `data-filter="reticulum"` ("RT") and `data-filter="pager"` ("Pager") buttons alongside the existing All/MT/MC/★ Fav in `frontend/js/messaging.js:49-55`; no JS logic changes needed (`setFilter` was already generic). Also added `flex-wrap` to `.msg-protocol-toggle` (`messaging.css`) so the now-6-button row wraps instead of overflowing on a narrow sidebar. Not yet seen live in a real browser. |
