...
| Propagation node | LXMF store-and-forward relay | `propagation_enabled` + `propagation_storage_limit_mb`; own `lxmf.propagation` hash, re-announced 6h; status line on Settings tab |
...
| Med | **Propagation node polish** | client-side "sync from a preferred PN" + transfer state; PN peering | Med — LXMF client API (`set_outbound_propagation_node`, `request_messages_from_propagation_node`) |
| ~~Med~~ | ~~Announce-stream tab~~ | done 2026-09-07 — Activity tab | |
| ~~Med~~ | ~~DM notifications~~ | done 2026-09-07 — `notify_url` | |
| ~~High~~ | ~~LXMF propagation node~~ | done 2026-09-07 — `propagation_enabled` (server side) | |
...
1. ~~Activity tab~~ ✓  ~~DM notifications~~ ✓  ~~Propagation node (server)~~ ✓
   (done 2026-09-07)
2. **Talk-back bot** — cheap, and it makes the `spacestate` / `events` data
   reachable over DM, not just the browsable pages.
3. Everything else as interest / need dictates. Audio is the big one and
