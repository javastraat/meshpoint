...
chips, in two passes, live-verified), item 2 (Paper messages / QR,
export-only, not yet Pi-tested), and item 3 (Audio calls -- **both
stages plus PTT**, a new `reticulum-call` plugin, live-verified both
directions ti↔vm, though two follow-up tweaks since that call haven't
been retested) — see **Done** below for all four. V6 (inbound-message
toast/sound notifications) also flagged and **live-verified same day** —
a real popup showed up when a message came in. Session builds through
2026-09-09: #1 Contacts,
...
| ~~3~~ | ~~**Audio calls**~~ (`call.audio`) | build | Med–High | High | Med | **BUILT + LIVE-VERIFIED 2026-09-16.** Both stages plus PTT; live two-node cross-device call confirmed working **both directions** (ti↔vm) after fixing a periodic-re-announce gap (`call.audio` was only ever announced once at startup, too cold to reliably propagate on a large mesh) — see Done log below for the full debugging writeup. Default mode changed 1200→3200, mic now requests autoGainControl/echoCancellation/noiseSuppression; neither retested live yet. |
...
| V3 | **Verify: Contacts tab + peer-drawer additions** | verify | Low | Med | — | **PARTIALLY VERIFIED 2026-09-16** — user walked the new Contacts list live on-device, confirmed it looks great. Still unconfirmed: inline-edit propagating name everywhere, add-from-hash + garbage rejected, Remove clears all surfaces, Send Message button pre-fills, favourite star syncs w/ Browse list, next-hop tooltip, light/dark |
...
| Companion plugin | **Reticulum Browser** (`reticulum-browser`, 2026-09-14) | Full multi-tab NomadNet browser — Networks-section sidebar page. Open several nodes at once, each tab keeps its own address/history; node picker + search (same shape as the reticulum plugin's Browse tab); favourites share the *exact same* `localStorage` list as the Browse tab, Peers drawer, and Dashboard's quick-browse modal — star anywhere, starred everywhere; raw/rendered Micron view toggle; keyboard shortcuts (Ctrl/Cmd+T/W/R, Alt+←/→); **form-field submission + `/file/...` downloads + identity fingerprinting, all BUILT 2026-09-16** (see "Could build" below — the last two were re-scoped from an "unscoped" placeholder to Low effort and built the same session, after the user added rBrowser's own source to the workspace so its real fingerprint mechanism could be read rather than guessed at). Feature set/name inspired by fr33n0w/rBrowser (MIT) — a clean-room implementation against Meshpoint's own patterns, not a code port; full credit in its README. `provides = ["sidebar"]` only, reads the reticulum plugin's public `/api/reticulum/nomad/*`; `requires = "reticulum"`. **Still NOT built** (its own README's "What's not here" section): a local NomadNet search engine + page cache (would need its own `service`-seam backend, comparable in scope to a separate plugin) |
