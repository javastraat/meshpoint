...
- **Publish node telemetry** (opt-in, `plugins.reticulum.telemetry_enabled`). Periodically send this box's own host stats — CPU/SoC temperature, load, RAM/disk, a status line — to a collector LXMF address as a Sideband-compatible `FIELD_TELEMETRY` frame, so a Sideband client subscribed to the node sees it on its telemetry screen. "Send telemetry now" button + `GET /api/reticulum/telemetry`. (v1 sends temperature + a status string; structured sensors and location later.)
