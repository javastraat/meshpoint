...
- **DAB+** — also a **bundled plugin** (`plugins/apps/dab/`), off by default; the last of the RTL-SDR decoders to be extracted, so Radio is the only Listener tab left built into core. Wraps [`welle-cli`](https://github.com/AlbrechtL/welle.io) (welle.io's headless DAB/DAB+ decoder) for browser playback — pick a channel/ensemble (e.g. `12C` for the NPO nationals), then a station once it's decoded; ★ favorites jump straight to a channel + station in one click. A companion **Config** panel shows every channel `dab_channel_scan.py` found on your antenna (name, SNR, station list), with an editable display-name override per channel that the player's channel picker also uses, and can run the scan itself with live streamed output. Unlike every other plugin here, DAB+ doesn't live on the Listener page — it's the first to migrate onto the newer **RTL-SDR Plugins** page instead (`plugins/apps/rtlsdr/`, also enable that), which is meant to eventually replace the Listener page entirely, one plugin at a time. Enable both with `plugins.rtlsdr.enabled: true` / `plugins.dab.enabled: true` after running `plugins/apps/dab/setup.sh` (a plain apt install, no build). `/api/dab/*` routes and both panels live in the plugin folder.
...
<a id="plugins"></a>
**Plugins**
- **App plugins** — optional features (an RTL-SDR listener + its API routes + a dashboard tab, a whole top-level sidebar page, or content injected into another plugin's page) that live in `plugins/apps/<id>/` instead of core, wired through a `register(reg)` entry point against a small facade over Meshpoint's route and listener registries. They load from `src/plugins/apps/` (bundled) or `plugins/apps/` (drop-in); a `plugin.toml` manifest declares the API version, capabilities (`listener`/`routes`/`panel`/`sidebar`/`hook`), apt/`setup.sh` dependencies, and frontend files. Loading is opt-in per plugin (`plugins.<id>.enabled` in `local.yaml`) because an in-process plugin runs with the service's privileges. The **ACARS** decoder is the reference plugin (`plugins/apps/acars/`) for `listener`/`routes`/`panel`; **RTL433** (`plugins/apps/rtl433/`), **ADS-B** (`plugins/apps/adsb/`), **P2000** (`plugins/apps/p2000/`), **POCSAG** (`plugins/apps/pocsag/`), **Pagers** (`plugins/apps/pagers/`) and **DAB+** (`plugins/apps/dab/`) are further examples, all extracted out of core the same way — P2000, POCSAG and Pagers were originally one shared file (`src/audio/pager_listener.py`) covering all three kinds, split out one at a time until nothing pager-related was left in core; DAB+ was the last RTL-SDR tab extracted, leaving Radio as the only one still built into core. **Hello World** (`plugins/apps/hello-world/`) is the reference for `sidebar` — a whole new top-level nav page, not just a Listener sub-tab; **Hello World Hook** (`plugins/apps/hello-world-hook/`) is the reference for `hook` — content injected into *another* plugin's already-rendered page (Hello World's own) instead of owning a page of its own, which a host page opts into via `window.mountPageHooks(...)`. See [Configuration → Plugins](docs/CONFIGURATION.md#plugins) for using one, or [docs/PLUGINS.md](docs/PLUGINS.md) for writing one.
...
- **Pagers, POCSAG, and P2000 (all optional):** three **plugins** (`plugins/apps/pagers/`, `plugins/apps/pocsag/`, `plugins/apps/p2000/`), none part of `scripts/install.sh` (it no longer builds `multimon-ng` at all — Pagers was the last thing in core using it). Each has its own `setup.sh` building [`multimon-ng`](https://github.com/EliasOenal/multimon-ng) from source (`sudo bash plugins/apps/<id>/setup.sh` or `sudo meshpoint plugin setup <id>`) — idempotent, so whichever of the three you set up first does the real work and the others are no-ops. Then set the matching `plugins.<id>.enabled: true` in `local.yaml`. See each plugin's own README ([`pagers`](plugins/apps/pagers/README.md), [`pocsag`](plugins/apps/pocsag/README.md), [`p2000`](plugins/apps/p2000/README.md)). Since Pagers/POCSAG/RTL433/ACARS/DAB+/ADS-B/P2000 use the same dongle as the Radio tab, only one can be active at a time; starting one while another is listening returns an error rather than silently stopping it.
...
- **DAB+ (optional):** also a **plugin** (`plugins/apps/dab/`), not part of `scripts/install.sh` — run `sudo bash plugins/apps/dab/setup.sh` (or `sudo meshpoint plugin setup dab`), a plain apt install of [`welle.io`](https://github.com/AlbrechtL/welle.io) (`--no-install-recommends`, since only its headless `welle-cli` binary is used, not the GUI app), then set `plugins.dab.enabled: true` in `local.yaml`. See [`plugins/apps/dab/README.md`](plugins/apps/dab/README.md).
