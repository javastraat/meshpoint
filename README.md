...
- **ACARS tab** via [`acarsdec`](https://github.com/f00b4r0/acarsdec) — decodes aircraft VHF datalink (ACARS) messages on the European channels 131.525 / 131.725 / 131.800 / 131.825 MHz into a live log: flight/tail, message label, and text, with recognised standard types (CPDLC, ADS-C, media advisory, ARINC formats) expanded via [`libacars`](https://github.com/szpajder/libacars). Start/stop/clear like the other listeners. New `/api/acars/*` endpoints; `scripts/install.sh` builds `acarsdec` + `libacars` from source.
...
| `POST /api/{p2000,pagers,pocsag,rtl433,acars}/start` · `.../stop` · `.../clear` | Start/stop/clear the P2000, Pagers, POCSAG, RTL433, or ACARS decoder (only one RTL-SDR listener runs at a time) |
