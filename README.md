...
- **ADS-B tab** via [`dump1090`](https://github.com/MalcolmRobb/dump1090) — live air traffic table (ICAO, flight, altitude, speed, track, position) from 1090 MHz transponder squitters, a "Metric units" toggle for meters/km-h instead of feet/knots, and a **Map** button (enabled once aircraft are tracked) opening a live OpenStreetMap (Leaflet) view with each aircraft plotted as a rotated plane icon colored by squawk, trailing its last couple of minutes of positions so you can see where it's been, and dropping off the map (though not the table) 60 seconds after its last update rather than lingering as a stale ghost for the full 5 minutes dump1090 itself keeps a contact around. Clicking any aircraft row, or a plane marker on the map, opens a **flight detail** modal — the tracked ADS-B fields plus, best-effort, registration/aircraft type/operator and route (origin–destination airport) from [hexdb.io](https://hexdb.io) and a real photo of the airframe from [planespotters.net](https://www.planespotters.net), both free public lookups queried straight from the browser. New `/api/adsb/*` endpoints.
...
- **P2000 / Pagers / POCSAG (optional):** for the dashboard's P2000 (Dutch emergency dispatch, FLEX), Pagers, and POCSAG tabs (all POCSAG-family decoders, on different bands), also install [`multimon-ng`](https://github.com/EliasOenal/multimon-ng) — not packaged in Debian/Raspberry Pi OS either, so build from source:
  ...
  ```
  Both `redsea` and `multimon-ng` are installed automatically by `scripts/install.sh` on a fresh Pi setup — this is only needed for a manual/non-installer setup. Since P2000/Pagers/POCSAG/RTL433/ACARS/DAB+/ADS-B use the same dongle as the Radio tab, only one can be active at a time; starting one while another is listening returns an error rather than silently stopping it.
...
- **ADS-B (optional):** for the dashboard's ADS-B tab, install [`dump1090`](https://github.com/MalcolmRobb/dump1090) — not packaged in Debian/Raspberry Pi OS, so build from source (its Makefile has no `install` target, and needs `-fcommon` on GCC 10+ or the link step fails with "multiple definition" errors from this codebase's pre-C11 tentative global definitions):
  ```bash
  git clone --depth 1 https://github.com/MalcolmRobb/dump1090.git /opt/dump1090
  cd /opt/dump1090 && make EXTRACFLAGS=-fcommon
  sudo install -m 755 dump1090 view1090 /usr/local/bin/
  ```
  Installed automatically by `scripts/install.sh` on a fresh Pi setup — manual build only needed for a non-installer setup. Confirm with `which dump1090` after installing.
...
| `GET /api/{p2000,pagers,pocsag}/status` | Decoder state: running, frequency, decoded messages |
| `POST /api/{p2000,pagers,pocsag}/start` · `.../stop` · `.../clear` | Start/stop/clear the P2000, Pagers, or POCSAG decoder (only one RTL-SDR listener runs at a time) |
| `GET/POST /api/{rtl433,acars}/...` | Same shape, provided by the RTL433/ACARS plugins when enabled (see [Plugins](#plugins)) — not present unless installed |
...
| `GET /api/adsb/status` | ADS-B listener state: running, current aircraft table (ICAO, flight, altitude, speed, track, position, seen) |
| `POST /api/adsb/start` · `.../stop` | Start/stop the ADS-B (dump1090) listener |
