...
- **P2000 / Pagers / POCSAG (optional):** for the dashboard's P2000 (Dutch emergency dispatch, FLEX), Pagers, and POCSAG tabs (all POCSAG-family decoders, on different bands), also install [`multimon-ng`](https://github.com/EliasOenal/multimon-ng) — not packaged in Debian/Raspberry Pi OS either, so build from source:
  ...
  ```
  Both `redsea` and `multimon-ng` are installed automatically by `scripts/install.sh` on a fresh Pi setup — this is only needed for a manual/non-installer setup. Since P2000/Pagers/POCSAG/RTL433 use the same dongle as the Radio tab, only one of the five can be active at a time; starting one while another is listening returns an error rather than silently stopping it.
...
| `POST /api/{p2000,pagers,pocsag,rtl433}/start` · `.../stop` | Start/stop the P2000, Pagers, POCSAG, or RTL433 decoder (only one of Radio/P2000/Pagers/POCSAG/RTL433 may run at a time) |
