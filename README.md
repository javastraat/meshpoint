...
- **DAB+ tab** — also a **bundled plugin** (`plugins/apps/dab/`), off by default; the last of the RTL-SDR tabs to be extracted, so Radio is the only Listener tab left built into core. Wraps [`welle-cli`](https://github.com/AlbrechtL/welle.io) (welle.io's headless DAB/DAB+ decoder) for browser playback — pick a channel/ensemble (e.g. `12C` for the NPO nationals), then a station once it's decoded; ★ favorites jump straight to a channel + station in one click. A companion **DAB+ Config tab** shows every channel `dab_channel_scan.py` found on your antenna (name, SNR, station list), with an editable display-name override per channel that the DAB+ tab's channel picker also uses, and can run the scan itself with live streamed output. Enable with `plugins.dab.enabled: true` after running `plugins/apps/dab/setup.sh` (a plain apt install, no build). `/api/dab/*` routes and both dashboard tabs live in the plugin folder.
