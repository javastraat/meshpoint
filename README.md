...
- **Pagers / POCSAG (optional):** for the dashboard's Pagers and POCSAG tabs (both POCSAG-family decoders, on different bands), also install [`multimon-ng`](https://github.com/EliasOenal/multimon-ng) — not packaged in Debian/Raspberry Pi OS either, so build from source:
  ```bash
...
| `GET /api/{pagers,pocsag}/status` | Decoder state: running, frequency, decoded messages |
| `POST /api/{pagers,pocsag}/start` · `.../stop` · `.../clear` | Start/stop/clear the Pagers or POCSAG decoder (only one RTL-SDR listener runs at a time) |
| `GET/POST /api/{rtl433,acars,adsb,p2000}/...` | Same shape, provided by the RTL433/ACARS/ADS-B/P2000 plugins when enabled (see [Plugins](#plugins)) — not present unless installed |
