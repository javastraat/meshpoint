...
- **The Reticulum page's Settings tab** edits display name, RNode radio (serial port dropdown from the shared USB-device picker, frequency/bandwidth/TX power/SF/CR) and the TCP backbone host/port; a "Restart rnsd" button applies RNode/backbone changes without a full meshpoint restart.
