...
- **Topbar theme toggle** — 9 themes (dark, light, high-contrast, sunlight, amber-mono, green-crt, Nord, Solarized Dark, Gruvbox Dark), each with a per-theme icon. Themes are drop-in folders — `frontend/themes/<id>/` with a `theme.json` + `theme.css`; the picker builds its list from `GET /api/themes`, so a new theme is "add a folder and restart", no code change. **Settings → Themes** sets the server-wide default and has a live theme builder that previews palette edits on the whole dashboard and downloads a ready-to-drop `theme.css` + `theme.json`.
...
- **P2000 / Pagers / POCSAG (optional):** for the dashboard's P2000 (Dutch emergency dispatch, FLEX), Pagers, and POCSAG tabs (all POCSAG-family decoders, on different bands), also install [`multimon-ng`](https://github.com/EliasOenal/multimon-ng) — not packaged in Debian/Raspberry Pi OS either, so build from source:
  ...
  ```
  Both `redsea` and `multimon-ng` are installed automatically by `scripts/install.sh` on a fresh Pi setup — this is only needed for a manual/non-installer setup. Since P2000/Pagers/POCSAG/RTL433/DAB+/ADS-B use the same dongle as the Radio tab, only one of the seven can be active at a time; starting one while another is listening returns an error rather than silently stopping it.
...
| `POST /api/{p2000,pagers,pocsag,rtl433}/start` · `.../stop` | Start/stop the P2000, Pagers, POCSAG, or RTL433 decoder (only one of Radio/P2000/Pagers/POCSAG/RTL433/DAB+/ADS-B may run at a time) |
