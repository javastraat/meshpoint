...
- **MeshCore repeater monitoring** (opt-in) — MeshCore nodes advertise identity only, so Meshpoint can actively poll repeaters you operate (`req_status`/`req_telemetry`, like the phone app) for battery, uptime, airtime, packet counters, noise floor and LPP sensors, shown on a dedicated **Repeaters** page; `repeater_poll:` in `local.yaml`.
