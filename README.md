...
- Native LXMF messaging — a persisted Reticulum identity, an `LXMRouter`, announce-based peer discovery, and send/receive, on the **Reticulum** sidebar page (Peers roster with colored aspect pills, People/Infrastructure counts, a flat Messages summary, an admin-only Send tab — optionally attach one image (≤5 MB) via LXMF's `FIELD_IMAGE`, shown as a thumbnail in the shared Messages chat view for both sides — and a Settings tab) plus a topbar pill. Lives entirely in a plugin (`provides = ["service", "routes", "sidebar", "topbar"]`) — the first user of the `"service"` seam (a lifespan-managed background service). Enable with `plugins.reticulum.enabled: true`. Structured to never conflict with hardware you already run — meshpoint's client attaches to a locally-running [`rnsd`](https://reticulum.network) shared instance rather than opening a physical radio interface itself.
