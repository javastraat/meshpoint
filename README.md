...
- **ADS-B tab** via [`dump1090`](https://github.com/MalcolmRobb/dump1090) — live air traffic table (ICAO, flight, altitude, speed, track, position) from 1090 MHz transponder squitters, a "Metric units" toggle for meters/km-h instead of feet/knots, and a **Map** button (enabled once aircraft are tracked) opening a live OpenStreetMap (Leaflet) view with each aircraft plotted as a rotated plane icon colored by squawk. Clicking any aircraft row opens a **flight detail** modal — the tracked ADS-B fields plus, best-effort, registration/aircraft type/operator and route (origin–destination airport) from [hexdb.io](https://hexdb.io) and a real photo of the airframe from [planespotters.net](https://www.planespotters.net), both free public lookups queried straight from the browser. New `/api/adsb/*` endpoints.
